function merge_Pandora_and_TropOMI()
...
save_fig = 1;
output_path = ['C:\Projects\TropOMI\plots\Wind_transport\' Pandora_no '\' method '\'];
...
%         data(~TF_wd,:) = [];
    ...
    case 'rotation'
        ...
        C_all = pair_Pandora_TropOMI_rotation(Pandora,TropOMI);
        ...
        [group_status.intercept(1),group_status.slop(1),group_status.slop_nlm(1),group_status.R(1),group_status.N(1)]  ...
            = auto_scatter(method,data,'Pandora','TropOMI','d1','original',save_fig,output_path,use_sum_no2);
        group_status.description(1) = {'original'};
        auto_scatter(method,data,'Pandora','TropOMI','t1','original',save_fig,output_path,use_sum_no2);   
        bin_by_x(method,data,'Pandora','TropOMI','original',save_fig,output_path,use_sum_no2);       
        
        [group_status.intercept(2),group_status.slop(2),group_status.slop_nlm(2),group_status.R(2),group_status.N(2)]  ...
            = auto_scatter(method,data,'Pandora','TropOMI','d1','up wind',save_fig,output_path,use_sum_no2);     
        group_status.description(2) = {'up wind'};
        auto_scatter(method,data,'Pandora','TropOMI','t1','up wind',save_fig,output_path,use_sum_no2);   
        bin_by_x(method,data,'Pandora','TropOMI','up wind',save_fig,output_path,use_sum_no2);
        
        [group_status.intercept(3),group_status.slop(3),group_status.slop_nlm(3),group_status.R(3),group_status.N(3)]  ...
            = auto_scatter(method,data,'Pandora','TropOMI','d1','down wind',save_fig,output_path,use_sum_no2);   
        group_status.description(3) = {'down wind'};
        auto_scatter(method,data,'Pandora','TropOMI','t1','down wind',save_fig,output_path,use_sum_no2);   
        bin_by_x(method,data,'Pandora','TropOMI','down wind',save_fig,output_path,use_sum_no2);
        ...
        switch bin_by
            case 'ws'
                ...
                TF4 = data.windspeed <= 10; % wind speed <= 10 km/hr
                group_status.description(4) = {'all ws <= 10 km hr-1'};
                
                TF5 = (data.windspeed > 10) & (data.windspeed <= 20); % wind speed in 10 to 20 km/hr
                group_status.description(5) = {'all ws 10 to 20'};
                
                TF6 = (data.windspeed > 20) & (data.windspeed <= 60);% wind speed in 20 to 60 km/hr
                group_status.description(6) = {'all ws 20 to 60'};
                
            case 'travel_t'
                sub_group_type = 'all';
                TF4 = abs(data.arrival_time) <= 1; % transport time <= 1 hr
                group_status.description(4) = {'all transport t <= 1 hr'};
                
                TF5 = (abs(data.arrival_time) > 1) & (abs(data.arrival_time) <= 2); % transport time within 1 to 2 hr
                group_status.description(5) = {'all transport t 1-2 hr'};
                
                TF6 = (abs(data.arrival_time) > 2) ; % transport time more than 2 hr
                group_status.description(6) = {'all transport t > 2 hr'};
                
            case 'travel_d'
                sub_group_type = 'all';
                TF4 = abs(data.y1) <= 5; % transport distance <= 5 km
                group_status.description(4) = {'all transport d <= 5 km'};
                
                TF5 = (abs(data.y1) > 10) & (abs(data.y1) <= 20); % transport distance within 10 to 20 km
                group_status.description(5) = {'all transport d 10-20 km'};
                
                TF6 = (abs(data.y1) > 20) ; % transport distance > 20 km
                group_status.description(6) = {'all transport d > 20 km'};
                
            case 'wd'
                sub_group_type = 'all';
                TF4 = (data.winddirection <= 60) | (data.winddirection >= 300); % wind direction -60 to 60 degree
                group_status.description(4) = {'all wd -60 to 60 degree'};
                
                TF5 = (data.winddirection >= 120) & (data.winddirection <= 240); % wind direction 120 to 240 degree
                group_status.description(5) = {'all wd 120 to 240 degree'};
                
                TF6 = (data.winddirection >= 150) & (data.winddirection <= 210); % wind direction 150 to 210 degree
                group_status.description(6) = {'all wd 150 to 210 degree'};                
        end
        ...
        [group_status.intercept(4),group_status.slop(4),group_status.slop_nlm(4),group_status.R(4),group_status.N(4)]  ...
            = auto_scatter(method,data_sub4,'Pandora','TropOMI','d1',sub_group_type,save_fig,output_path,use_sum_no2,cellstr(group_status.description(4))); 

        [group_status.intercept(5),group_status.slop(5),group_status.slop_nlm(5),group_status.R(5),group_status.N(5)]  ...
            = auto_scatter(method,data_sub5,'Pandora','TropOMI','d1',sub_group_type,save_fig,output_path,use_sum_no2,cellstr(group_status.description(5))); 

        [group_status.intercept(6),group_status.slop(6),group_status.slop_nlm(6),group_status.R(6),group_status.N(6)]  ...
            = auto_scatter(method,data_sub6,'Pandora','TropOMI','d1',sub_group_type,save_fig,output_path,use_sum_no2,cellstr(group_status.description(6))); 
        
        fig_name = ['group_status_' bin_by];
        figure;hold all;
        ax1 = subplot(3,1,1);        
    ...
    case 'wind_driven'    
        ...
        C_closest_orginal = pair_Pandora_TropOMI(Pandora,TropOMI,delta_t,up_down_original);
        print_setting(1/4,save_fig,[output_path fig_name]);

        up_down_original = 'up wind';% 'original', 'up wind', 'down wind'
        fig_name = ['scatter_' up_down_original];
        C_closest_up_wind = pair_Pandora_TropOMI(Pandora,TropOMI,delta_t,up_down_original);
        print_setting(1/4,save_fig,[output_path fig_name]);

        up_down_original = 'down wind';% 'original', 'up wind', 'down wind'
        fig_name = ['scatter_' up_down_original];
        C_closest_down_wind = pair_Pandora_TropOMI(Pandora,TropOMI,delta_t,up_down_original);
        print_setting(1/4,save_fig,[output_path fig_name]);
...
%%
function C_output = pair_Pandora_TropOMI_rotation(Pandora,TropOMI)
% simple QC
TF_qa = TropOMI.qa == 1;
TropOMI(~TF_qa,:) = [];
...
%idx_original = (abs(TropOMI.y1) <= y_dis) & (hours(abs(TropOMI.arrival_time)) <= hours(1)) ;
idx_original = (abs(TropOMI.y1) <= y_dis) & (hours(abs(TropOMI.arrival_time)) < hours(1)) ;
...
for i = 1:numel(means_serial_need_loop)
    ...
    [~,index_min] = min(abs(sub_C_nonunique.arrival_time));
...
%%
function C_closest = pair_Pandora_TropOMI(Pandora,TropOMI,delta_t,up_down_original)
% simple QC
TF_qa = TropOMI.qa == 1;
TropOMI(~TF_qa,:) = [];
...
line_fits(x,y);
scatter(x,y,10,C_closest.distance_4pair,'filled');
ylabel('TropOMI NO_2 VCD [DU]');
xlabel('Pandora NO_2 VCD [DU]');
...
%%
function [status,bins_all] = bin_by_x(method,data,x_name,y_name,type,save_fig,output_path,use_sum_no2)
step = 0.1; % step in x
...
figure;hold all;
fig_name = ['binned_scatter_' method '_' y_name '_vs_' x_name '_' type(~isspace(type))];
x = status.x_centre;
y = status.y_mean;
